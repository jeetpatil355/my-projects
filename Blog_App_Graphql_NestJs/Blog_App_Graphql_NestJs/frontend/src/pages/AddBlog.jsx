...
import { IMG_BLOG_MUTATION, CREATE_BLOG_MUTATION } from '../mutations/blog';
        ...
        try {
            ...
            if (pic.length === 0) {
                ...
                const result = await createBlog({
                    variables: {
                        title, content, pic, category, is_private, id
                    },
                });

                if (result.data.createBlog.status === 'success') {
                    toast.success(result["data"].createBlog.data)
                    navigate('/add-blogs')
                } else {
                    toast.error(result['data'].createBlog.data)
                }
                clearField()
            }
        } catch (er) {
