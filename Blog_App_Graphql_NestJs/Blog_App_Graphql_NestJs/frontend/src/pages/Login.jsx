...
const Login = ({ setIsLogin }) => {
  ...
  const handleSubmit = async (event) => {
    try {
      ...
      const result = await await loginUserMutation({
        variables: {
          email,
          password,
        },
      ...
      if (result.data.Login.status === "success" && check === true) {
        toast.success(result.data.Login.data);

        localStorage["token"] = result.data.Login.token;
        localStorage["id"] = result.data.Login.id;
        setIsLogin(true);
        navigate("/");
      }
    } catch (er) {
