...
router.get('/my/:userId', async (request, response) => {
...
router.put('/:id', async (request, response) => {
  ...
  try {
    await db.execute(
      `update note set title = ?, details = ?, dueDate = ? where id = ?`,
      [title, details, dueDate, id]
    )
    response.send(utils.createSuccess('done'))
  } catch (ex) {
    response.send(utils.createError(ex))
...
router.patch('/public/:id', async (request, response) => {
  ...
  try {
    await db.execute(`update note set isPublic = 1 where id = ?`, [id])
    response.send(utils.createSuccess('done'))
  } catch (ex) {
    response.send(utils.createError(ex))
  }
