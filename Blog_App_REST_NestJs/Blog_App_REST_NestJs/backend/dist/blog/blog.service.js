...
let BlogService = class BlogService {
    ...
    async updateBlog(id, title, content, pic, category, isPrivate, request) {
    ...
    async markBlogAsPrivate(blogId, userId) {
        await this.blogRepository.update({ blog_id: blogId, user: { id: userId } }, { is_private: 1 });
        return { status: 'success', data: 'Marked Private' };
    }
    async markBlogAsPublic(blogId, userId) {
        await this.blogRepository.update({ blog_id: blogId, user: { id: userId } }, { is_private: 0 });
